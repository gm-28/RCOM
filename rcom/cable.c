...
// CABLE modes
#define CABLE_MODE_OFF   0
#define CABLE_MODE_ON    100
#define CABLE_MODE_NOISE 200
...
void addNoiseToBuffer(char *buf, int size);
...
int main(int argc, char** argv)
{
    ...
    printf("\n\n"
           ...
           "--- noise : add noise to the cable\n"
           "--- end   : terminate the program\n"
           "\n");

    // Configure serial ports
    int fdTx = open("/dev/emulatorTx", O_RDWR | O_NOCTTY);
    if (fdTx < 0) {
    ...
    configureSerialPort(fdTx, &oldtioTx, &newtioTx);

    int fdRx = open("/dev/emulatorRx", O_RDWR | O_NOCTTY );
    if (fdRx < 0) {
    ...
    configureSerialPort(fdRx, &oldtioRx, &newtioRx);

    // Configure stdin to receive commands to this program
    int oldf = fcntl(STDIN_FILENO, F_GETFL, 0);
    fcntl(STDIN_FILENO, F_SETFL, oldf | O_NONBLOCK);

    char tx2rx[BUF_SIZE] = {0};
    char rx2tx[BUF_SIZE] = {0};
    char rxStdin[BUF_SIZE] = {0};

    int cableMode = CABLE_MODE_ON;
    ...
    while (STOP == FALSE) {
        ...
        if (bytesFromTx > 0) {
            if (cableMode == CABLE_MODE_OFF) {
                printf("bytesFromTx=%d > bytesToRx=CONNECTION OFF\n", bytesFromTx);
            } else {
                if (cableMode == CABLE_MODE_NOISE)
...
void addNoiseToBuffer(char *buf, int size)
{
    // Flip all bits of the first byte
    buf[0] = buf[0] ^ 0xFF;
}
